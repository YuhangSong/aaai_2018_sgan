...
\begin{eqnarray}\label{p-tau}
    ...
    \end{cases}
\end{eqnarray}
Here, $\delta$ is also approaching to zero. We assume $\delta$ approaches zero in the same order as $\varepsilon$ approaching zero\footnote{Note that in practice, $\varepsilon$ may approach zero in a much more higher order than $\delta$ approaching zero, i.e., $\varepsilon=a\delta^b$. Since $\varepsilon$ is the minimal data value we can have on a computer, while $\delta$ is a hyper-parameter we set to be as large as possible. But this does not effect the conclusion we will have in \eqref{limit-constant}.}.
