...
\section{The Proposed $H$ Distance}

We consider the one dimensional case as a start. Following is the optimal $H$ distance proposed in place of Wasserstein distance,
...
\begin{equation}\label{H-distance}
  H^{\ast}(x,\mathbb{P}_r,\mathbb{P}_g)=| \int_{x}^{1} \mathbb{P}_r(x') d x' - \int_{x}^{1} \mathbb{P}_g(x') d x' |
\end{equation}
where $x_{r}$ are real samples sampled from distribution $\mathbb{P}_{r}$, and $x_{g}$ are generated samples sampled from distribution $\mathbb{P}_{g}$,
...
\end{equation}
Note that both $x_{r}$ and $x_{g}$ are normalized between $[0,1]$, so that the integral operation in \eqref{H-distance} can be conducted in $[0,1]$, instead of $[-\infty,+\infty]$. This distance is modeled for every $x$, which means we train the generative model $g$ to minimize $H^{\ast}$ at every $x_g$,
...
For every $x_{r}, x_{g}$ pair, we sample $x_{\epsilon}$ between $x_{r}$ and $x_g$,
\begin{equation}\label{x-epsilon}
  x_{\epsilon} = \epsilon x_{r} + (1-\epsilon) x_g
\end{equation}
where
\begin{equation}\label{epsilon}
  \epsilon \sim Uniform[0,1]
\end{equation}
Assuming $x_{\epsilon}$ follows a distribution given by $\mathbb{P}_{\epsilon}$,
\begin{equation}\label{x-epsilon}
  x_{\epsilon} \sim \mathbb{P}_{\epsilon}
\end{equation}
Apparently, $\mathbb{P}_{\epsilon}$ can be represented by $\mathbb{P}_r$ and $\mathbb{P}_g$, but in a tricky way.





Then we build a model $D^{\theta}$ with parameter vector $\theta$. For every set of $\{ x_{r}, x_{g}, x_{\epsilon} \}$, the update rule of $\theta$ is,
\begin{equation}\label{D-loss}
  \theta \longrightarrow \theta + \nabla_{\theta} \|\nabla_{x_{\epsilon}}D^{\theta}(x_{\epsilon})-\frac{x_{r}-x_{g}}{\|x_{r}-x_{g}\|}\|^2
\end{equation}
...
where $x_{\epsilon}$ is a random variable computed by,
